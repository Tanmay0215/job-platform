...
const Internships = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${BACKEND_URL}/api/internships`);
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-10 px-4 sm:px-6">
      <div className="max-w-7xl mx-auto">
        <h1 className='text-center text-3xl md:text-4xl font-medium p-5'>INTERNSHIPS</h1>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {data && data.map((item, index) => (
              <div key={index} className="bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition duration-200">
                ...
                <button onClick={() => navigate(item.applicationLink)}
